...
import polars as pl
from toolkitx.task_utils import with_resilience, PersistentTaskQueue
...
def test_queue_enqueue_dataframe(temp_db_path, sample_dataframe):
    ...
    import sqlite3
    conn = sqlite3.connect(temp_db_path)
    cursor = conn.execute(f"SELECT COUNT(*) FROM test_enqueue WHERE status='pending'")
...
def test_queue_enqueue_idempotent(temp_db_path, sample_dataframe):
    ...
    import sqlite3
    conn = sqlite3.connect(temp_db_path)
    cursor = conn.execute(f"SELECT COUNT(*) FROM test_idempotent")
    count = cursor.fetchone()[0]
...
def test_queue_process_success_tasks(temp_db_path, sample_dataframe):
    ...
    assert set(result_df["name"].to_list()) == {"Apple", "Google", "Microsoft", "Amazon"}
    assert all(result_df["is_company"] == True)
...
def test_queue_process_max_retries_exceeded(temp_db_path, error_dataframe):
    ...
    import sqlite3
    conn = sqlite3.connect(temp_db_path)
    cursor = conn.execute(f"SELECT COUNT(*) FROM test_max_retry WHERE status='failed'")
...
def test_queue_crash_recovery(temp_db_path, sample_dataframe):
    ...
    import sqlite3
    conn = sqlite3.connect(temp_db_path)
    conn.execute(f"UPDATE test_recovery SET status='processing' WHERE status='pending'")
    conn.commit()
    conn.close()
...
def test_queue_concurrent_execution(temp_db_path, sample_dataframe):
    ...
    assert total_time < 0.3, f"Expected < 0.3s with concurrency, got {total_time:.2f}s"
...
def test_queue_graceful_shutdown(temp_db_path, sample_dataframe):
    ...
    def run_queue():
        ...
        original_process = queue.process
        ...
        def modified_process(worker_func, concurrency=10):
            ...
            with queue._get_conn() as conn:
                cursor = conn.execute(f"SELECT COUNT(*) FROM {queue.table_name} WHERE status='pending'")
            ...
            try:
                ...
                if queue._shutdown_event.is_set():
                    with queue._get_conn() as conn:
                        conn.execute(f"UPDATE {queue.table_name} SET status='pending' WHERE status='processing'")
                        conn.commit()
    ...
    completed = conn.execute(
        f"SELECT COUNT(*) FROM test_shutdown WHERE status='completed'"
    ).fetchone()[0]
    
    # processing 任务应该被恢复为 pending
    pending = conn.execute(
        f"SELECT COUNT(*) FROM test_shutdown WHERE status='pending'"
    ).fetchone()[0]
