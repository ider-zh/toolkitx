...
TENCENT_KEYS_AVAILABLE = bool(
    os.getenv("TENCENT_API_ID") and os.getenv("TENCENT_API_KEY")
)
...
def test_translator_init_baidu_with_direct_params(
    temp_cache_path, mock_baidu_engine_class
):
    """测试使用直接参数初始化百度翻译引擎"""
    ...
    mock_baidu_engine_class.assert_called_once_with(
        api_id=TEST_BAIDU_API_ID_PARAM, api_key=TEST_BAIDU_API_KEY_PARAM
    )
    translator.close_cache()


def test_translator_init_baidu_with_monkeypatched_env(
    temp_cache_path, mock_baidu_engine_class, monkeypatch
):
    ...
    assert translator.engine_name == "baidu"
    mock_baidu_engine_class.assert_called_once_with(
        api_id="env_baidu_id_monkey", api_key="env_baidu_key_monkey"
    )
    translator.close_cache()
...
def test_translator_init_tencent_with_direct_params(
    temp_cache_path, mock_tencent_engine_class
):
    """测试使用直接参数初始化腾讯翻译引擎"""
    translator = Translator(
        engine="tencent",
    ...
    mock_tencent_engine_class.assert_called_once_with(
        api_id=TEST_TENCENT_API_ID_PARAM, api_key=TEST_TENCENT_API_KEY_PARAM
    )
    translator.close_cache()


def test_translator_init_tencent_with_monkeypatched_env(
    temp_cache_path, mock_tencent_engine_class, monkeypatch
):
    ...
    assert translator.engine_name == "tencent"
    mock_tencent_engine_class.assert_called_once_with(
        api_id="env_tencent_id_monkey", api_key="env_tencent_key_monkey"
    )
    translator.close_cache()
...
def test_translator_init_tencent_missing_keys_error(temp_cache_path, monkeypatch):
    ...
    with pytest.raises(
        ValueError, match="Tencent Secret ID and Secret Key are required"
    ):
...
@pytest.mark.skipif(
    not BAIDU_KEYS_AVAILABLE,
    reason="BAIDU_API_ID and/or BAIDU_API_KEY not set in environment (e.g., via .env)",
)
def test_translate_baidu_with_env_keys_cache_miss_and_hit(
    temp_cache_path, mock_baidu_engine_class
):
    ...
    translator = Translator(
        engine="baidu", cache_path=temp_cache_path
    )  # 不传递 api_id/key

    # 验证 BaiduTranslation 构造函数是否用环境变量中的密钥被调用
    mock_baidu_engine_class.assert_called_once_with(
        api_id=expected_api_id, api_key=expected_api_key
    )
    ...
    assert (
        result1 == "translated_from_baidu"
    )  # 这是 mock_engine_instance.translate 的返回值
    mock_baidu_engine_instance.translate.assert_called_once_with(
...
@pytest.mark.skipif(
    not TENCENT_KEYS_AVAILABLE,
    reason="TENCENT_API_ID and/or TENCENT_API_KEY not set in environment (e.g., via .env)",
)
def test_translate_tencent_with_env_keys_cache_miss_and_hit(
    temp_cache_path, mock_tencent_engine_class
):
    ...
    mock_tencent_engine_instance.translate.assert_called_once_with(
        text, target_lang, source_lang
    )
...
@pytest.mark.skipif(not BAIDU_KEYS_AVAILABLE, reason="Baidu keys not in .env")
def test_translate_uses_default_langs_with_env_keys(
    temp_cache_path, mock_baidu_engine_class
):
    ...
    mock_engine_instance.translate.assert_called_once_with(
        text, default_target, default_source
    )
    translator.close_cache()
...
def test_close_cache_direct_params(
    temp_cache_path, mock_baidu_engine_class
):  # 这个测试不需要依赖环境变量
    ...
    with patch.object(
        translator.cache, "close", wraps=translator.cache.close
    ) as mock_cache_close:
        translator.close_cache()
